...
| T12 | Register `claim_crisis_event` in Tauri handler | Agent | 🟡 Fix applied → independent verification required | `claim_crisis_event` added to `generate_handler!` in `src-tauri/src/lib.rs`. |
| T13 | Remove duplicate `export_user_data` registration | Agent | 🟡 Fix applied → independent verification required | Duplicate removed from Data Management section in `lib.rs`. |
...
| T15 | Verify and fix: does crisis detection in `chat_with_mentor` actually call `create_crisis_event`? | Agent | 🟡 Fix applied → E2E/regression verification required | Verified broken: it did not call `create_crisis_event`. Fix applied in `ai.rs` to explicitly persist a `CrisisEvent` (severity 'high', status 'pending') before returning the crisis response. Failures in persistence return an explicit error. Regression test `test_crisis_detection_persists_event` added. |

...
| T15 | Verify and fix: does crisis detection in `chat_with_mentor` actually call `create_crisis_event`? | Agent | 🟡 / ✅ Source-verified, CI pending | **`c49b790` independently verified by source inspection and mutation analysis; T15-1 through T15-4 protect the orchestration invariant. Local execution remains blocked by native build dependencies. CI execution pending.** |

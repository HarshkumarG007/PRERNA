...
| T15 | Verify and fix: does crisis detection in `chat_with_mentor` actually call `create_crisis_event`? | Agent | ✅ Verified (Pending CI) | **T15 — ✅ Verified — commit `c49b790`; independent inspection confirms tests run against `chat_with_mentor_inner` (the orchestration path), `ActiveSession` supplies `user_id` without spoofing potential, crisis DB failure propagates correctly due to a valid fault-injection integration test, and benign messages do not escalate. A mutation thought-test confirms that bypassing the crisis delegation in the command causes T15-1 to fail. Awaiting CI run to confirm compilation.** |
